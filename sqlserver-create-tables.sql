CREATE TABLE tbl_funcionario(
    codigo_funcionario integer IDENTITY(1,1) PRIMARY KEY,
...
INSERT INTO tbl_funcionario_telefone(codigo_funcionario, numero_telefone)
  SELECT codigo_funcionario, CAST(codigo_funcionario AS varchar(5)) FROM tbl_funcionario WHERE nome LIKE 'Func%';

CREATE TABLE tbl_dependente (
    codigo_funcionario INTEGER REFERENCES tbl_funcionario (codigo_funcionario),
    nome VARCHAR(50) NOT NULL,
	idade INTEGER NOT NULL CHECK (idade BETWEEN 0 AND 100),
	PRIMARY KEY (codigo_funcionario, nome)
);
...
CREATE TABLE tbl_curso (
    codigo_curso INTEGER IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE tbl_disciplina (
    codigo_disciplina INTEGER IDENTITY(1,1) PRIMARY KEY,
...
CREATE TABLE tbl_aluno (
    codigo_aluno INTEGER IDENTITY(1,1) PRIMARY KEY,
	nome varchar(50) NOT NULL,
	rua  VARCHAR(100) NOT NULL,
	numero INTEGER,
	complemento VARCHAR(50),
...
--Quantidade de professores por disciplina
    SELECT nome_disciplina,
	       COUNT(f.codigo_funcionario) AS quantidade_professores
      FROM tbl_professor_disciplina AS pd
INNER JOIN tbl_funcionario f ON (pd.codigo_funcionario = f.codigo_funcionario)
INNER JOIN tbl_professor p ON (pd.codigo_funcionario = p.codigo_funcionario)
INNER JOIN tbl_disciplina d ON (pd.codigo_disciplina = d.codigo_disciplina)
